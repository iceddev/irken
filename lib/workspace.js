...
var localforage = process.browser ? require('localforage') : null;
...
function Workspace(){
  ...
  structure.on('swap', function(n, o, updatedPath){
    ...
    if(updatedPath.indexOf('cwd') === 0){
      ...
      // fire this off and save async in the background
      self.userConfig('cwd', self.cwd);
      return;
    }
...
// so it doesn't use immutable objects yet
Workspace.prototype.userConfig = function userConfig(key, value, cb){
  if(!localforage){
    return
  }
